...
class MinesweeperView(Frame):
    def __init__(self, model, controller, parent=None):
        ...
        Button(panel, text='Новая игра',
               command=self.controller.startNewGame).pack(side=RIGHT)
            ...
            for column in range(self.model.columnCount):
                btn = Button(
                    ...
                    command=lambda row=row,
                                   column=column: self.controller.onLeftClick(
                        row, column),
                    padx=0,
                    pady=0
                )
                ...
                btn.bind(
                    '<Button-3>',
                    lambda e, row=row,
                           column=column: self.controller.onRightClick(row,
                                                                       column)
                )
